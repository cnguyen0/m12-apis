...
GetReview <- function(movie) {
  ...
  api.key <- "2d3963db35bb4d0f88f17a193b8c62e8" # you would need to request this thru the website
  query.params <- list(query=movie, api_key = api.key)
  response <- GET(base.url, query = query.params)
  body <- content(response, "text")
  
  # Request data using your search query
  results <- fromJSON(body) # turning the query we got and changing that into a json file.
