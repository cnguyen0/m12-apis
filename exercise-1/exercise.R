...
response <- GET("http://data.unhcr.org/api/documents/countries.json")
body.countries <- content(response, 'text')
countries <- fromJSON(body.countries)
countries$country_code[countries$name == 'Syrian Arab Republic']
...
response <- GET("http://data.unhcr.org/api/stats/persons_of_concern.json?year=2013&country_of_origin=AFG&country_of_residence=CHE")
